...
print("=== ADD POINTS_USED MIGRATION - Fix SQLAlchemy column error ===")
...
print("\n✓ Adding points_used column to student_exam_unlocks (CRITICAL FIX)")
cur.execute("""
ALTER TABLE student_exam_unlocks 
ADD COLUMN IF NOT EXISTS points_used INTEGER DEFAULT 0;
""")

print("✅ SUCCESS: points_used column added/verified")
print("\nMigration complete! This fixes 'column student_exam_unlocks.points_used does not exist'")
print("🔄 Restart FastAPI: uvicorn main:app --reload")
print("🧪 Test: POST /api/admin/unlock/process/20")

cur.close()
conn.close()
print("Migration ready. Run: python add_points_used_migration.py")
