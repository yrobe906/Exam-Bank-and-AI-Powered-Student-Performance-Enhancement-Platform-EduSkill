      ...
      <div v-if="!canSubmit && !submitting" class="submit-disabled-hint">
        ...
        <div>
          ...
          <ul>
            ...
            <li v-if="unlockMethod === 'payment' && ['cbe','boa'].includes(selectedPaymentMethod) && !paymentProofFile">📸 Upload payment screenshot</li>
          </ul>
  ...
  computed: {
    canSubmit() {
      console.log('canSubmit check:', {
        ...
        canUnlock: this.eligibility?.can_unlock,
      ...
      // **FIX**: Points require sufficient XP
      if (this.unlockMethod === 'points') {
        return this.eligibility.can_unlock;
      }
      if (this.unlockMethod === 'payment') {
        return this.selectedPaymentMethod && (!['cbe','boa'].includes(this.selectedPaymentMethod) || this.paymentProofFile);
      }
      return false;
    }
  },
  async mounted() {
    console.log('PremiumFixed mounted, query:', this.$route.query);
  ...
  methods: {
    ...
    async submitRequest() {
      ...
      try {
        ...
        if (this.unlockMethod === 'points') {
          ...
          // **NEW**: XP Confirmation Dialog
          if (!confirm(`Deduct ${points_req} XP points?\n\n✅ Immediate deduction\n⏳ Pending admin approval\n❌ Refund on reject`)) {
            return;
          }
          
          response = await unlockApi.payWithPoints(api, this.contentId, points_req);
          this.$toast?.success(`✅ ${points_req} XP deducted! Pending admin approval`);
        } else {
          ...
          this.$toast?.success(`Payment unlock request submitted successfully`);
        }
        
        await this.loadMyRequests();
        await this.loadProfile(); // Reload to show updated XP
        await this.checkEligibility(); // Re-check if now unlocked
        this.currentRequestId = response.data?.id;
        
        // Start polling for status if pending
        if (response.data?.status === 'pending') {
          this.startPolling(response.data.id);
        }
      } catch(e) {
        console.error('Submit error:', e);
        this.errorMessage = e.response?.data?.detail || `Failed to submit ${this.unlockMethod} request`;
        this.showError = true;
      } finally {
    ...
    startPolling(requestId) {
      this.pollingInterval = setInterval(async () => {
        try {
          const req = await unlockApi.getRequest(api, requestId);
          if (req.data.status !== 'pending') {
            clearInterval(this.pollingInterval);
            await this.loadMyRequests();
            await this.loadProfile();
            this.$toast?.success(`Request ${req.data.status}!`);
          }
        } catch(e) {
          clearInterval(this.pollingInterval);
        }
