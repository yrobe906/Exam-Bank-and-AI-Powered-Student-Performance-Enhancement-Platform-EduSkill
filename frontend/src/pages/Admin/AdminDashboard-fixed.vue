<template>
  <div class="min-h-screen flex bg-[radial-gradient(circle_at_top_left,_rgba(255,255,255,1),_rgba(248,250,252,1),_rgba(226,232,240,1))] text-slate-900">
    ...
    <div class="fixed top-0 left-0 right-0 z-50 h-20 bg-white/95 backdrop-blur-xl border-b border-slate-200 shadow-sm">
      <MainHeader class="h-full" @toggle-sidebar="sidebarOpen = !sidebarOpen" />
    </div>
    ...
    <!-- Main Content -->
    <div :class="[
      'flex-1 flex flex-col pt-20 min-h-screen transition-all duration-300 ease-in-out md:ml-80',
...
export default {
  ...
  setup() {
    ...
    const handleLogout = () => {
      ...
      // sessionStorage clears automatically when browser is closed/reopened
      sessionStorage.removeItem('admin_token')
      sessionStorage.removeItem('admin_user')
